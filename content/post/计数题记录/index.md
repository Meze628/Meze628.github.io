...
> 小 R 在剩余最后 $2$ 元的时候，此时 $a_i>\frac{a_j}{2}>a_k$ 且 $a_j>a_i+a_k$，小 R 的选择不是最优的    
...
- $[k+1,n]$ 中的物品不会被买到，定价随意。     

我们只需计算 $m-2$ 用于分配的方案数，第一段每个先给保底 $1$ 元，还剩 $m-2-(j-1)=m-j-1$ 元，接着这些钱要分配给第一段中 $w=2$ 的物品和第二段中 $w=1$ 的物品各 $1$ 元。方案数是 $\binom{i-2}{m-j-1}$，第三段不用考虑，第四段的方案数是 $2^{n-k+1}$，因为 $w_k$ 的取值也是随意的，因此对于一组 $(i,j)$，能够造出：
$$
